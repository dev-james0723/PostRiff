...
class PostgresWorker:
    ...
    def complete(self, claimed, result):
        ...
        if not isinstance(result, dict) or result.get("state") not in allowed or not isinstance(result.get("confirmed"), str):
            result = {"state": "uncertain", "confirmed": "Malformed adapter response; reconcile before retry"}
        with self.connection_factory() as db:
            with db.cursor() as cur:
