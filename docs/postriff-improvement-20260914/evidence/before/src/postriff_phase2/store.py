...
class Phase2Store(Store):
    ...
    def mutate(self, wid, token, expected_revision, action, payload):
        ...
        with self.connect() as db:
            ...
            device = db.execute("SELECT id,user_id FROM alpha_devices WHERE workspace_id=? AND credential_hash=?", (wid, hashlib.sha256(token.encode()).hexdigest())).fetchone()
    ...
    def apply_phase2(self, db, s, action, p, device):
        ...
        if action == "plan":
            ...
            if v["blockedByRetraction"] or v["voiceRevision"] != s["speaker"]["activeRevision"] or any(not self._source(s, i)["active"] for i in v["sourceIds"]):
            ...
            job = {"id": uid(), "manifest": copy.deepcopy(manifest), "approvalDigest": review["digest"], "approvedAt": now, "approvedBy": device["user_id"], "state": "approved", "events": [], "attempts": [], "checks": 0, "leaseOwner": None, "leaseUntil": 0, "nextAt": manifest["timing"]["timestamp"], "cancelRequested": False}
            ...
            for review in reviews:
                ...
                self.apply_phase2(db, s, "approve", {**review, "confirmed": True}, device)
        elif action == "cancel":
    ...
    def build_manifest(self, s, p, actor):
        ...
        if v["needsReview"] or v["blockedByRetraction"] or v["unknowns"] or any(not self._source(s, i)["active"] for i in v["sourceIds"]):
    ...
    def worker_step(self, owner="local-worker", crash=None):
        ...
        with self.connect() as db:
            ...
            if not isinstance(result, dict) or result.get("state") not in allowed:
                result = {"state": "uncertain", "confirmed": "Malformed adapter response; reconcile"}
