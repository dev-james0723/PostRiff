...
def validate_structure(root=ROOT):
    ...
    private_exclusions = all(marker in exclusions for marker in (".env*", "broker.key", "*.command", "src/james_au_social/**", ".venv/**"))
    ...
    upload_boundary = all(rule in upload_rules for rule in (".env*", ".*-broker-*", ".upgrade-*", "broker.key", "src/james_au_social/", "studio/broker/", "studio/web/node_modules/", "docs/", "tests/")) and not any(rule.startswith("!") for rule in upload_rules)
    ...
    for relative in ("api/index.py", "src/postriff_phase2/media.py", "src/postriff_phase2/hosted_app.py"):
    ...
    checks.append(_result("production-cron", "pending", "choose Vercel plan and required scheduling latency before adding a production cron cadence"))
