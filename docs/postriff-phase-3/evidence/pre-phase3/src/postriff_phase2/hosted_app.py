...
def supabase_verifier(project_url, publishable_key):
    ...
    return SupabaseSessionCandidate(project_url, get_user).verify
...
def runtime_from_environment(environ=None):
    ...
    verify = supabase_verifier(project_url, publishable)
    storage = PrivateAssetService(SupabaseStorage(project_url, secret))
    service = HostedWorkspaceService(database, verify, storage)
