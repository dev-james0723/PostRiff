...
class RuntimeAdapter:
 ...
 def __init__(self,route,timeout_seconds=60):
  require(route in self.versions,'Unknown adapter.',400);self.route=route;self.process=None;self.partial=[];self.timeout_seconds=min(60,max(.05,timeout_seconds))
 def inspect(self):
  ...
  with tempfile.TemporaryDirectory(prefix='postriff-run-') as directory:
   ...
   def reader():
    while True:
     ...
     messages.put(line)
     if not line:return
   thread=threading.Thread(target=reader,daemon=True);thread.start();started=time.monotonic();size=0;candidate=None
   try:
    while time.monotonic()-started<self.timeout_seconds:
     ...
     if raw.get('type')=='result' and isinstance(raw.get('structured_output'),dict):candidate=raw['structured_output']
    else:raise TimeoutError('Runtime timed out; partial text retained.')
    require(self.process.wait(timeout=3)==0 and candidate is not None,'No valid completed artifact.',409)
    return artifact(candidate,manifest)
   finally:
...
class ManagedWriting:
 ...
 def start(self,manifest,permit,emit):
  ...
  body={'model':permit.model,'messages':[{'role':'system','content':SYSTEM},{'role':'user','content':json.dumps(manifest,ensure_ascii=False)}],'max_tokens':2048,'temperature':.3,'response_format':{'type':'json_object'},'stream':False}
  ...
  value=artifact(json.loads(result['choices'][0]['message']['content']),manifest)
  emit({'type':'completed'})
  return {'artifact':value,'usage':{'provenance':'provider_reported','values':self._usage(result)}}
 @staticmethod
 def _usage(result):
