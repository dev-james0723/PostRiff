...
- **History rewrite, 2026-09-16 07:02 (not a release)**: `git filter-repo` purged an accidentally committed packaged app build (`desktop/artifacts/**`, 377 files) from every commit on `main` and `consumer-saas`; trees are otherwise identical and authorship is intact. Commit ids quoted above map to the rewritten history as `eb527b3 → 60b9947` (production), `b7adc41 → 3907ec5`, `b209ef6 → 38461e3`, `3937767 → 815ccd8`, `ac46caf → 08038aa`, `977ae97 → fb3ef30`, `929bf29 → 498c99f`, `fcd778e → b12684b`, `1724bad → 839f41a`, `fab0c54 → 57e39f2`, `13d913d → f1c81f2`. Deployments were built from clean exports, so nothing on Vercel changed. `origin/consumer-saas` was pushed at `af9d178` afterwards.
