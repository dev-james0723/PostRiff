...
- James 試「write a post about the most advanced model of Suno」：model 冇 approved facts 所以拒絕，但用 prose 講原因，schema 入面 `variants` 空 → UI 只見「returned no LinkedIn · English candidate」。已修：prompt 教 model 拒絕時都要回 JSON（該 destination 唔入 `variants`，`warnings` 逐項講需要乜）；`normalize_output` 會將 `warnings` 或 model 嘅 prose 變成失敗訊息「did not draft. It needs: …」；Codex route 同樣。真正令佢寫到嘅係 sources——所以同日做埋 Phase 5 第一片（下面）。

**Phase 5 第一片：agent 自己上網搵 facts（2026-09-16，James 明確要求：唔可以「停低」，要去搵）**
- `src/postriff_phase2/research.py`：`needs_research(text, intent, has_facts)`——message 有 URL、或 intent 係 research、或（冇 approved facts 而且唔係第一身 idea）就搵；`query_for` 剝走「write me a post about … for LinkedIn」之類指令淨返題目；`ExaSearch`（Exa 嘅 MCP server，即 agent-reach 用嗰個，plain HTTP JSON-RPC，冇 key，一定要有 User-Agent）→ 過濾 walled hosts（x / instagram / facebook / tiktok / linkedin / youtube / reddit…）→ `JinaReader`（r.jina.ai 讀成乾淨 markdown）→ `paragraphs()` 抽最多 12 段、每段 ≤ 500 字做 facts；最多 2 頁、總時限 30 秒；任何失敗都係 warning，唔會令 turn 死。可用 `POSTRIFF_RESEARCH=0` 關、`POSTRIFF_EXA_MCP_URL` / `POSTRIFF_READER_URL` 改 backend。
- `ideas.py` `_research()`：喺 run transaction 之前跑（network I/O）；每頁用現有 `source`（kind text，title = 「頁題 — host」）＋ `approve_source` 入 workspace，`origin = {kind: web_research, url, host, query, published, fetchedAt}`，policy 用 text 嘅預設 `rewrite_approval`（即 draft 係 candidate，發佈前要 James approve use），`egressConsent` 加 `cloud`（公開網頁可以去任何 route）；同一頁（fingerprint）唔會入兩次；`sourceIds` 加埋新 source；events：`progress.updated stage=researched`、每頁 `source.added`、research warnings；assistant body ＋ run usage 記 `research`（query、pages、sourceIds）；每個 variant 加 warning「Some facts came from web research (hosts); check them against the pages before scheduling」。
