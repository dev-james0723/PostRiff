...
| Personalized voice drafting | same | same | adds the approved voice profile and permitted voice samples for the exact route, or for every Rafii AI writer model when the owner allowed that class (the draft records the exact route); without one it drafts neutral with a note | same | same | existing voice tests + `postgres_voice_*` | NOT_RUN |
