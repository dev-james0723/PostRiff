...
## Internet and social research via Agent Reach

When a request asks to search, research, look up, or scrape the internet,
shares a URL, or asks what people or posts say on a supported social app, load
the global `agent-reach` skill before source acquisition and use it for
read-only fetch/routing. This includes public web pages, articles/RSS,
discussion search, public profiles/posts/comments, GitHub, and supported
YouTube/Bilibili/podcast metadata or captions. Agent Reach is acquisition only:
never use it to post, comment, like, follow, message, upload, schedule, delete,
or engage.

- Run `agent-reach doctor --json` before any multi-backend or login-backed
  platform. Follow `active_backend` and the matching reference retry chain, and
  announce the selected platform/backend before fetching.
- Prefer an installed dedicated James/channel skill when it owns the exact read
  operation; otherwise use Agent Reach's supported route. It does not add
  coverage to unsupported channels, qualify a provider, or bypass a provider
  gate. YouTube/Bilibili/video still use `james-au-video-transcript-intake`,
  exact public article URLs use `james-au-source-extraction-providers`, and
  mainland trend intelligence preserves RedFox's read-only contract.
- Record the query or exact URL, platform, backend, retrieval time, result
  identifiers/URLs, represented scope, access/rights limits, and failures in
  `james-au-research-and-source-log`. Treat output as acquisition or discovery,
  not factual truth, exhaustive coverage, James's opinion, or reuse permission.
  Verify current or high-stakes claims from authoritative sources and do not
  copy posts or retrieve private data.
- Keep Agent Reach temporary output in `/tmp` and persistent configuration in
  `~/.agent-reach`. If its CLI or a needed backend is missing, report
  `research_backend_unavailable` and stop or use only a separately approved
  route; never silently claim Agent Reach was used. After substantial
  multi-platform research, run `agent-reach check-update` after collection,
  not mid-task.
