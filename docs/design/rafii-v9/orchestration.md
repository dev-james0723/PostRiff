...
| Production logs (new deployment) | There were 0 error, fatal or 5xx entries. The cron returned 200 every minute (358–482 ms). Its only warning is the pre-existing `cron.completed` "unavailable", caused by migration 019 not being applied. |
...
| Remaining | Not applied, and not needed by this release: 019 (the cron warning), 016 (API tokens), the 014/015 policies, 017 and a migration ledger. Also outstanding: the provider review flags and live transport before any publishing, a signed-in production pass by the owner, and physical-device checks. |
