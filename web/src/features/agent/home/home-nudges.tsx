...
/** Neutral starting points (DNA §21.1): no personal projects, brands or sample data. Each label is an example request, so the capability shows itself. */
export const STARTING_POINTS: { id: string; label: string; text: string }[] = [
  { id: 'behind', label: 'A behind-the-scenes post', text: 'A behind-the-scenes thought: the quiet, imperfect work is usually where the best ideas begin. Turn this into a short post in my words.' },
  { id: 'lesson', label: 'What I learned this week', text: 'One thing this week taught me about creating: consistency matters more than waiting for inspiration. Draft this as a post that ends with a question for readers.' },
  { id: 'research', label: 'Research posting cadence', text: 'Research the latest guidance on posting cadence for small creators and log every source before writing a short summary post.' },
  { id: 'plan', label: 'Plan this week’s posts', text: 'Plan this week: one post per selected channel, spread across Tuesday and Thursday mornings, from the notes below.' }
];

export function StartingPoints({ onPick, disabled }: { onPick: (text: string) => void; disabled?: boolean }) {
  return (
    <div role='group' className='text-muted-foreground flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-xs' aria-label='Example requests'>
        ...
        <div className='flex flex-col gap-3 pb-1'>
          ...
          <div className='grid gap-2.5 sm:grid-cols-2'>
            {items.map((item) => {
              ...
              return (
                <button key={item.id} type='button' aria-pressed={active} disabled={disabled} onClick={() => onPick(item)} title={`Usually: ${item.usually}`} className={cn('rafii-focus flex min-h-14 flex-col gap-1 rounded-[var(--rafii-radius-card)] p-3.5 text-left transition-colors', active ? 'rafii-glass-selected' : 'rafii-quiet hover:rafii-glass')}>
                  <span className='text-foreground text-sm font-medium'>{item.title}</span>
                  <span className='text-muted-foreground line-clamp-2 text-xs leading-relaxed'>{item.explanation}</span>
                </button>
              );
            })}
          </div>
