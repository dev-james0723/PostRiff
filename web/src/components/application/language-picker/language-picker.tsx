...
function saveRecent(tag: string) {
  try {
...
function groupsFor(query: string, suggestions: { tag: string; reason: string }[], recent: string[]) {
  if (query.trim()) {
 ...
 */
export function LanguagePicker({ trigger, title, selected, onPick, suggestions = [], actions = [], disabled }: LanguagePickerProps) {
              ...
              {group.rows.map((row) => {
                ...
                return (
                  ...
                  >
                    ...
                    {entry.regionless && <span className='shrink-0 rounded-full bg-amber-500/10 px-1.5 text-[10.5px] text-amber-700 dark:text-amber-300'>No region</span>}
