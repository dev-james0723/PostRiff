...
export interface BouncyAccordionProps {
  ...
  /** PostRiff: wrap each trigger in a heading of this level, so an FAQ keeps its question headings. */
...
function BouncyAccordionRow({
  ...
  reduce: boolean | null;
  classNames?: BouncyAccordionClassNames;
  headingLevel?: BouncyAccordionProps['headingLevel'];
  onToggle: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      >
        {Heading ? (
          <Heading className='m-0 font-[inherit] text-[length:inherit]'>{trigger}</Heading>
        ) : (
          trigger
        )}
