...
function DefaultIcon({
  ...
  if (item.type === 'file') return <Icons.page className='size-4' />;
  if (reduce) {
    return open ? <Icons.folderOpen className='size-4' /> : <Icons.folder className='size-4' />;
  }
  ...
  return (
    <AnimatePresence initial={false} mode='popLayout'>
      ...
      >
        {open ? <Icons.folderOpen className='size-4' /> : <Icons.folder className='size-4' />}
      </motion.span>
    </AnimatePresence>
  );
}
