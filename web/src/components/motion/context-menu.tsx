...
export function ContextMenuTrigger({
  ...
  const interactiveTrigger =
    (typeof children.type === 'string' &&
      ['a', 'button', 'input', 'select', 'summary', 'textarea'].includes(children.type)) ||
    ['button', 'link', 'menuitem', 'tab', 'treeitem', 'option'].includes(
      String((childProps as { role?: string }).role ?? '')
    );
  ...
  return cloneElement(children, {
    ...
    // PostRiff: popup state only belongs on a control. On an article or figure
    // (a pipeline card, a library image) these attributes are invalid ARIA.
    ...(interactiveTrigger
      ? {
    ...
    onContextMenu: (event: ReactMouseEvent<HTMLElement>) => {
      ...
      if (event.defaultPrevented || disabled) return;
      // PostRiff: right-clicking text the reader selected inside the trigger keeps
      // the browser's own menu, so Copy and Look Up stay one click away.
    ...
    >
      <motion.div
        ...
        animate={{
          ...
          clipPath:
            context.reduce || context.modality === 'keyboard' || visualOpen ? clipShown : clipHidden
        }}
        transition={
...
export function ContextMenuCheckboxItem({
  ...
  return (
    <ContextMenuItemBase
      {...props}
      // oxlint-disable-next-line jsx-a11y/role-has-required-aria-props -- ContextMenuItemBase renders `ariaChecked` as aria-checked
      ...
      <span className='flex h-4 w-4 shrink-0 items-center justify-center'>
        <AnimatePresence initial={false}>
          {checked ? (
            ...
            >
              <Icons.check aria-hidden='true' className='h-3.5 w-3.5' strokeWidth={2.4} />
            </motion.span>
          ) : null}
        </AnimatePresence>
      </span>
