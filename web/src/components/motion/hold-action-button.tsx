...
> {
  ...
  /** PostRiff: colour of the liquid edge (a `text-*` class); match it to the fill. */
  waveClassName?: string;
  labelClassName?: string;
}

export const HoldActionButton = forwardRef<HTMLButtonElement, HoldActionButtonProps>(
  function HoldActionButton(
    ...
    // PostRiff: a button disabled mid-hold never hears its release, so drop an
    // unfinished hold here. A completed hold keeps its label while work runs.
    useEffect(() => {
      if (disabled && !completedRef.current) setHolding(false);
    }, [disabled]);
    ...
    // PostRiff: park the fill a full wave height past its edge, so the liquid edge
    // (1.25rem beyond the fill) never peeks into a short button at rest.
    const idleTransform =
      type === 'horizontal'
        ? 'translateX(calc(-100% - 1.25rem))'
        : 'translateY(calc(100% + 1.25rem))';
            ...
            {!reduce ? (
              type === 'horizontal' ? (
                <motion.svg
                  ...
                  className={cn('absolute -right-5 top-0 h-[200%] w-6 text-sky-400', waveClassName)}
                >
                  <path
                ...
                <motion.svg
                  ...
                  className={cn('absolute -top-5 left-0 h-6 w-[200%] text-sky-400', waveClassName)}
                >
                  <path
