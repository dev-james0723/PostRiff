...
export function NotificationStack({
  ...
  const countLabel = `${items.length} notification${items.length === 1 ? '' : 's'}`;
  ...
  if (!primaryItem) {
    return (
      ...
      >
        <Icons.bellOff className='h-4 w-4' aria-hidden='true' />
        {emptyLabel}
      </div>
    );
  }
  ...
  return (
    <motion.button
      ...
      aria-label={
        isExpanded ? `${countLabel}. ${expandedLabel}.` : `${countLabel}. Expand notifications.`
      }
