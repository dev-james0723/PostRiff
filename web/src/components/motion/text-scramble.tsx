...
export interface TextScrambleProps {
  ...
  style?: CSSProperties;
  /**
   * PostRiff: scramble when `text` changes (default). Pass false for changes the reader did not
   * make, such as a stored preference arriving after mount, so they settle without the effect.
   */
  animate?: boolean;
}
...
export function TextScramble({
  ...
  useEffect(() => {
    ...
    if (reduce || !glyphs || !shouldAnimate) {
      setDisplay(text);
      return;
    }
    ...
    // `animate` is read at the moment `text` changes; toggling it alone never starts a scramble.
