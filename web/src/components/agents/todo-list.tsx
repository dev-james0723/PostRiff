...
export interface TodoListProps {
  ...
  maxHeight?: number;
  className?: string;
  /** PostRiff: accessible name for the list section (defaults to the title when it is text). */
  ariaLabel?: string;
  /**
   * PostRiff: spin the in-progress ring (the default, for work that is running). Set false when
   * "in progress" only means "the current step", so nothing looks busy that is not.
   */
  spinActive?: boolean;
}
    ...
    >
      ...
      <motion.circle
        ...
        animate={{
          ...
          rotate: status === 'in-progress' && progress === undefined && spin && !reduce ? 360 : -90
        }}
        transition={
          status === 'in-progress' && progress === undefined && spin && !reduce
...
export function TodoList({
  ...
  return (
    <section
      aria-label={ariaLabel ?? (typeof title === 'string' ? title : 'Agent task list')}
              ...
              <AnimatePresence initial={false} mode='popLayout'>
                {items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <TodoStatusIcon status={status} progress={item.progress} spin={spinActive} />
