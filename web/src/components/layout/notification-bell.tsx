  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger render={<Button variant='ghost' size='icon' aria-label={label} className='relative' />}>
        ...
        {count > 0 && (
          <span className='bg-foreground text-background absolute -top-0.5 -right-0.5 min-w-4.5 rounded-full px-1 text-[11px] leading-[1.125rem] font-semibold tabular-nums'>{count}</span>
        )}
      </PopoverTrigger>
