# Modified: restrict upload hosts; fail on chunk loss rather than silently finalizing.
r"""
模块：video
功能：获取视频各种信息以及操作视频
...
def get_danmaku_view(page_id: int, bvid: str = None, aid: int = None, verify: utils.Verify = None):
    ...
    :param aid:
    :param bvid:
    :param page_id: 分p id，请先调用 get_video_info() ，然后取其中的 ["pages"][分P号-1]["cid"]
    :param verify:
    :return:
    """
    if verify is None:
...
def get_danmaku(bvid: str = None, aid: int = None, page_id: int = 0,
    ...
    """
    dms = []
    g = get_danmaku_g(bvid, aid, page_id, verify, date)
    for d in g:
        dms.append(d)
    return dms


def get_danmaku_g(bvid: str = None, aid: int = None, page_id: int = 0,
                verify: utils.Verify = None, date: datetime.date = None):
    """
    获取弹幕，返回生成器
    :param aid:
...
def video_upload(path: str, verify: utils.Verify, on_progress=None):
    ...
    # 上传设置
    try:
        resp = session.get('https://member.bilibili.com/preupload', params={'r': 'probe'}, timeout=30)
        probe_result = resp.json()
        probe_params = urllib.parse.parse_qs(probe_result["lines"][0]["query"])
        if on_progress:
            on_progress({"event": "PRE_UPLOAD_PROBE", "ok": True, "data": probe_result})
    except Exception as e:
        if on_progress:
            on_progress({"event": "PRE_UPLOAD_PROBE", "ok": False, "data": e})
        raise e
    params = {
    ...
    try:
        resp = session.get('https://member.bilibili.com/preupload', params=params, timeout=30)
        settings = resp.json()
        upload_urls = [
            'https:' + endpoint + '/' + settings['upos_uri'].replace('upos://', '')
            for endpoint in settings['endpoints']
        ]
        for target in upload_urls:
            parsed = urllib.parse.urlparse(target)
            if parsed.scheme != "https" or not parsed.hostname or not parsed.hostname.endswith((".bilivideo.com", ".bilivideo.cn")):
                raise ValueError("untrusted_upload_host")
        headers = {
        ...
        raise e
    try:
        resp = session.post(upload_urls[0] + "?uploads&output=json", headers=headers, timeout=30)
    ...
    async def upload(chunks: collections.deque, sess, upload_url):
        failed_chunks = []
            ...
            while True:
                ...
                try:
                    async with sess.put(upload_url, params=chunk, data=f.read(chunk['size']), headers=utils.DEFAULT_HEADERS) as r:
                        if r.status != 200:
                            raise aiohttp.client.ClientError(f"error code: {r.status}")
                        else:
                            if on_progress:
    ...
    async def main():
        ...
        chunks_settings_queue = collections.deque(chunks_settings)

        async with aiohttp.ClientSession(
                headers={'X-Upos-Auth': settings['auth']},
                cookies=verify.get_cookies(),
                timeout=aiohttp.ClientTimeout(total=7200)  # Larger timeout
        ) as sess:
            chs = task_chunks
            url_id = 0
            while True:
                ...
                for _ in range(settings['threads']):
                    coroutines.append(upload(chunks_settings_queue, sess, upload_urls[url_id]))
                    url_id = (url_id + 1) % len(upload_urls)
                ...
                if len(failed_chunks) == 0:
                    break
                raise exceptions.UploadException("chunk_upload_failed")
            # 验证是否上传成功
            params = {
                'output': 'json',
                'name': os.path.basename(path),
            ...
            async with sess.post(upload_urls[0], params=params, data=payload) as resp:
