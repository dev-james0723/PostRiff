...
def get_root_time(root_xml):
    record_info = root_xml.find('BililiveRecorderRecordInfo')
    if record_info is not None:
        ...
        record_info = root_xml.find("metadata")
        if record_info is None:
            raise Exception("can not find 'BililiveRecorderRecordInfo' or 'metadata' node")
        record_start_time_str = record_info.find('record_start_time').text
        record_start_time = parse(record_start_time_str)
        return record_start_time
