...
import { AccountMenu, AnalyticsLimited, AudienceLimited, ChannelsCloud, IdeasCloud, UsagePlan } from "./Cloud";
...
export default function FounderApp() {
  ...
  const [authMode, setAuthMode] = useState<"local" | "supabase" | "dev">("local");
  const [execution, setExecution] = useState("");
  const [downloads, setDownloads] = useState(0);
  const [reviewField, setReviewField] = useState<string | null>(null);
  // Milestone E shell state: collapsible sidebar, utility menu, mobile More sheet, OAuth return, voice setup mode.
  const [collapsed, setCollapsed] = useState(() => readLocal<boolean>("postriff-sidebar-collapsed", false));
  const [accountOpen, setAccountOpen] = useState(false);
  const [moreOpen, setMoreOpen] = useState(false);
  const [setupMode, setSetupMode] = useState(false);
  const [oauthReturn, setOauthReturn] = useState<{provider: string; state: string; code?: string; error?: string} | null>(() => {
    if (typeof window === "undefined" || window.location.pathname !== "/channels/connect") return null;
    const q = new URLSearchParams(window.location.search);
    const provider = q.get("provider"), oauthState = q.get("state");
    window.history.replaceState({}, "", "/");
    return provider && oauthState ? { provider, state: oauthState, code: q.get("code") || undefined, error: q.get("error") || undefined } : null;
  });
  const state = snapshot?.state;
  const hosted = authMode !== "local";
  const cloudMode = hosted && !!state && !state.workspace.sample;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const mode = c.authMode === "supabase" ? "supabase" : c.authMode === "dev" ? "dev" : "local";
        if (active) {
          setTemplates(c.templates);
          setProfileMetadata(c.profileMetadata);
          setPhase2(Boolean(c.phase2));
          setAuthMode(mode);
          setExecution(c.execution || "");
        }
        let current = access;
        if (mode === "dev" && current && current.authMode !== "dev") current = null; // stale local access on a dev API
        if (oauthReturn && current) setNav("Channels");
        if (mode === "supabase") {
          ...
          if (session && !current) {
            ...
            if (active) {
              setAccess(current);
              setSnapshot(created);
              setNav(oauthReturn ? "Channels" : created.state.session.completed ? "Dashboard" : "Ideas");
            }
        ...
        if (current) {
          ...
          if (active) {
            setSnapshot(saved);
            // An OAuth return must land on Channels so the exchange completes before anything else.
            setNav(oauthReturn ? "Channels" : saved.state.session.completed ? "Dashboard" : "Ideas");
          }
        }
      } catch (e) {
        if (active)
  ...
  function toggleCollapsed() {
    ...
    setCollapsed(next);
    writeLocal("postriff-sidebar-collapsed", next);
  }
  ...
  const cloudProps = access && snapshot ? { access, state: snapshot.state, revision: snapshot.revision, busy, onError: setError, onNotice: setNotice, reload: reloadSnapshot } : null;
  ...
  async function deleteHostedAccount(confirmation: string) {
    if (!access || authMode !== "supabase") return;
    await api.deleteAccount(access, confirmation);
    localStorage.removeItem(ACCESS_KEY);
    localStorage.removeItem("postriff-alpha-personal-access");
    setAccess(null);
    setSnapshot(null);
  ...
  if (authOpen)
    content = (
      <AuthEntry
        phase2={phase2}
        authMode={authMode === "local" ? undefined : authMode}
  ...
  else if (cloudMode && cloudProps && nav === "Ideas" && !setupMode)
    content = <IdeasCloud key={state.workspace.id + "ideas-cloud"} {...cloudProps} openSetup={() => setSetupMode(true)} />;
  else if (cloudMode && cloudProps && nav === "Channels")
    content = <ChannelsCloud key={state.workspace.id + "channels-cloud"} {...cloudProps} oauthReturn={oauthReturn} clearReturn={() => setOauthReturn(null)} />;
  else if (cloudMode && cloudProps && nav === "Usage")
    content = <UsagePlan key={state.workspace.id + "usage"} {...cloudProps} />;
  else if (cloudMode && cloudProps && nav === "Analytics")
    content = <AnalyticsLimited key={state.workspace.id + "analytics"} {...cloudProps} />;
  else if (cloudMode && cloudProps && nav === "Audience")
    content = <AudienceLimited key={state.workspace.id + "audience"} {...cloudProps} />;
        ...
        account={
          state.phase2 ? (
            <Privacy
              ...
              deleteHostedAccount={deleteHostedAccount}
            />
          ) : (
            <Settings
              embedded
  ...
  const accountItems = [
    { label: "You · voice & profile", hint: "Speaker identity, preferences", onSelect: () => go("You") },
    { label: "Skills library", hint: "Released skill templates", onSelect: () => go("Skills") },
    ...(cloudMode ? [{ label: "Usage & Plan", hint: "Allowance, spend, receipts", onSelect: () => go("Usage") }] : []),
    ...(cloudMode && setupMode ? [{ label: "Back to Ideas conversation", onSelect: () => { setSetupMode(false); go("Ideas"); } }] : []),
    ...(state ? [{ label: "Export drafts", hint: "ZIP with receipt", onSelect: () => void exportPack() }] : []),
    ...(access ? [{ label: "Sign out", danger: true, onSelect: () => void clearSession() }] : []),
  ];
  const navSymbols = ["◫", "✳", "▦", "⇄", "▥", "◎"];
  const mobilePrimary = ["Dashboard", "Ideas", "Scheduling", "Audience"];
  return (
    <div className={`alpha-app ${collapsed ? "collapsed" : ""}`}>
      ...
      >
        <button className="collapse-toggle" aria-label={collapsed ? "Expand navigation" : "Collapse navigation"} aria-pressed={collapsed} onClick={toggleCollapsed}>
          {collapsed ? "»" : "«"}
        </button>
        <a
          className="wordmark"
        ...
        <div className="workspace-label">
          ...
          <span>
            ...
            <small>
              {phase2
                ? hosted
                  ? "Phase 2 · hosted private beta"
                  : "Phase 2 · local fixtures"
                : "Private founder alpha"}
            </small>
          </span>
          <span
            className="local-dot"
            aria-label={hosted ? "Hosted private workspace" : "Local only"}
          />
        </div>
        ...
        <div className="sidebar-bottom">
          <div className="local-card">
            ...
            <span>
              {hosted ? "Private cloud workspace" : "Only on this device"}
              <small>
                {hosted ? "Saved in your private workspace." : "Your ideas stay here."}
              </small>
            </span>
          </div>
        </div>
      ...
      <div className="app-body">
        <div className="topbar">
          ...
          <div className="topbar-right">
            {state && !state.workspace.sample && (
              <button className="create-button" onClick={() => { setSetupMode(false); go("Ideas"); }} aria-label="Create: open Ideas">
                <span aria-hidden="true">+</span>
            ...
            <div className="account-anchor">
              <button className="account-button" aria-label="Account and workspace menu" aria-haspopup="menu" aria-expanded={accountOpen} onClick={() => setAccountOpen(!accountOpen)}>
                <span className="avatar small" aria-hidden="true">
                  M
                </span>
              </button>
              <AccountMenu open={accountOpen} onClose={() => setAccountOpen(false)} items={accountItems} />
            </div>
          </div>
        </div>
        {authMode === "dev" && (
          <div className="dev-banner" role="status">
            ...
            Local dev harness · identity and providers are synthetic · {execution || "dev-synthetic"} · nothing leaves this machine
          </div>
        )}
        ...
        <main
          className={`main ${state && !authOpen && !state.workspace.sample && nav === "Ideas" && step !== 5 && step !== 6 && (!cloudMode || setupMode) ? "with-progress" : ""}`}
          aria-busy={disabled}
        >
          <div className="main-content">
            ...
            {state?.phase3 && access && snapshot && !authOpen && (nav === "Ideas" || nav === "You") && <RuntimePanel key={state.workspace.id} state={state} access={access} revision={snapshot.revision} act={act} onSnapshot={setSnapshot} busy={busy} />}
            {content}
                ...
                (state.workspace.sample
                  ...
                  : busy
                    ? hosted
                      ...
                      : "Saving locally…"
                    : hosted
                      ? `Confirmed state saved in your private workspace · revision ${snapshot?.revision}`
        ...
        <footer className="app-footer">
          ...
          <span>
            {hosted ? "Private hosted beta · v0.1" : "Private local prototype · v0.1"}
          </span>
        </footer>
      </div>
      {state && !authOpen && (
        <nav className="bottom-nav" aria-label="Primary">
          {mobilePrimary.map((item) => (
            <button key={item} aria-current={nav === item ? "page" : undefined} onClick={() => go(item)}>
              <span className="nav-symbol" aria-hidden="true">{navSymbols[NAV.indexOf(item)]}</span>
              {item}
            </button>
          ))}
          <button aria-current={["Channels", "Analytics", "Usage", "You", "Skills"].includes(nav) ? "page" : undefined} aria-haspopup="dialog" aria-expanded={moreOpen} onClick={() => setMoreOpen(true)}>
      ...
      {moreOpen && (
        <div className="more-sheet" role="dialog" aria-modal="true" aria-label="More" onClick={() => setMoreOpen(false)}>
          <div className="sheet-body" onClick={(e) => e.stopPropagation()}>
            <h2>More</h2>
            {["Channels", "Analytics"].map((item) => (
              <button key={item} className="row" onClick={() => go(item)}>
                <span>{item}</span>
                <span className="nav-symbol" aria-hidden="true">{navSymbols[NAV.indexOf(item)]}</span>
              </button>
            ))}
            {accountItems.map((it) => (
              <button key={it.label} className="row" onClick={() => { setMoreOpen(false); it.onSelect(); }}>
                <span>{it.label}</span>
              </button>
            ))}
          </div>
