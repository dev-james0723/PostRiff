...
class AudienceService:
    def __init__(self, repository, oauth, clock, transport=None):
    ...
    def ingest_replies(self, cur, workspace_id, connection_id, provider, provider_post_id, now):
        if provider != "threads" or self.transport is None:
    ...
    def threads(self, workspace_id, token):
        with self.repository.transaction(token, workspace_id) as (cur, row, _):
            cur.execute("SELECT id::text,connection_id,provider,provider_post_id,provider_comment_id,author_handle,text,extract(epoch from ingested_at),tombstoned_at IS NOT NULL FROM public.pr_audience_threads WHERE workspace_id=%s ORDER BY ingested_at DESC LIMIT 200", (workspace_id,))
            items = []
            for r in cur.fetchall():
                reply_level = self._capability(cur, workspace_id, r[1], "reply")
                items.append({"threadId": r[0], "connectionId": r[1], "provider": r[2], "providerPostId": r[3], "commentId": r[4], "author": r[5], "text": r[6], "ingestedAt": float(r[7]), "tombstoned": bool(r[8]), "replyAvailable": reply_level == "Direct" and self._member(row).allows("reply"), "replyLevel": reply_level})
            ...
            return {"threads": items, "capabilities": capability, "limits": "Automated or bulk replies and moderation are not available in this release; each reply is approved individually."}
    ...
    def draft_reply(self, workspace_id, token, thread_id, payload):
        ...
        if origin not in ("manual", "ai_fixture"):
            raise AlphaError("Choose a manual or AI-assisted draft.")
        with self.repository.transaction(token, workspace_id) as (cur, row, principal):
            ...
            return {"draftId": cur.fetchone()[0], "origin": origin, "text": text, "label": "AI-suggested (deterministic preview)" if origin == "ai_fixture" else "Your reply"}
    ...
    def reply_preview(self, workspace_id, token, draft_id):
        with self.repository.transaction(token, workspace_id) as (cur, _, _):
            ...
            return {"manifest": manifest, "digest": digest(manifest), "action": f"Send this reply as {account[0] if account else 'an unconnected account'}", "replyLevel": self._capability(cur, workspace_id, r[2], "reply")}
    ...
    def approve_reply(self, workspace_id, token, draft_id, manifest_digest, confirmed):
        ...
        with self.repository.transaction(token, workspace_id) as (cur, row, principal):
            ...
            cur.execute("UPDATE public.pr_reply_drafts SET status='approved',approval=%s::jsonb,events=events||%s::jsonb,updated_at=now() WHERE id::text=%s AND workspace_id=%s AND status='draft'", (json.dumps({"digest": manifest_digest, "approvedBy": principal, "at": self.clock(), "manifest": preview["manifest"]}), json.dumps([{"at": self.clock(), "state": "approved"}]), draft_id, workspace_id))
            if cur.rowcount != 1:
            ...
            return {"draftId": draft_id, "status": "approved", "note": "Queued for the connector worker; sending and verification are recorded separately."}
