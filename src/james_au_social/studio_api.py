...
class LocalOwnerBoundary:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # A leased image has a random 256-bit path capability, expires quickly,
        # and is still constrained to this loopback host. The native RedNote
        # driver may fetch it as a cross-site image without Studio's session.
        is_xiaohongshu_asset_lease = bool(re.fullmatch(
            r"/api/xiaohongshu-publishing/assets/[A-Za-z0-9_-]{43}", path,
        ) and method == "GET" and not scope.get("query_string"))
        if (site is not None and site not in ("none", "same-origin")
                and not safe_connection_return and not is_xiaohongshu_asset_lease):
        ...
        try:
            ...
            authenticated = False
        if is_api and not is_health and not is_xiaohongshu_asset_lease and not authenticated:
...
def create_app(data_dir: Path, project_root: Path, port: int = 4310, provider=None, delivery_supervisor=None, connection_broker=None):
    ...
    def bootstrap():
        ...
        if connection_broker is not None:
            states={
                ...
                'facebook': connection_broker.facebook_status(),
                ...
                'x': connection_broker.x_status(),
                # LinkedIn has its own local broker.  Keep the Studio summary
                # aligned with the dedicated LinkedIn connection panel.
                'linkedin': connection_broker.linkedin_status(),
            ...
            result['capabilities']['scheduling']=bool(
                states['youtube'].get('publishReady')
                or states['facebook'].get('publishReady')
                or states['linkedin'].get('schedulingAvailable')
            )
        return result
    ...
    from .x_browser import register_routes as register_x_browser_routes
    register_x_browser_routes(app, data_dir, _body)
    from .studio_connections import ConnectionBroker, register_connection_routes
    broker = connection_broker or ConnectionBroker(project_root, data_dir, port)
    register_connection_routes(app, broker, _body)
    from .studio_xiaohongshu_publish import register_xiaohongshu_publishing
    app.state.xiaohongshu_publishing = register_xiaohongshu_publishing(app, store, broker, _body, port)
