"""Approval and idempotency ledger for the persistent Codex X browser route.

The authenticated session is owned by Codex's in-app browser. This module never
reads cookies and never launches a second browser profile.
"""
...
class Store:
    ...
    def status(self):
        c=self.get('connection') or {}; connected=bool(c.get('profile') and c.get('username'))
        return dict(provider='x',route='controlled_browser',routeDriver=VERSION,state='connected_identity' if connected else 'login_or_verification_required',
                    profile=c.get('profile'),username=c.get('username'),identitySignals=c.get('identitySignals',[]),
                    sessionState='verify_before_each_action',publishReady=bool(connected and c.get('liveTestVerified')),
                    publishing=False,liveTestVerified=bool(c.get('liveTestVerified')),cost='free_no_api')
    ...
    def work(op,v):
        ...
        try:
            ...
            if op=='login':
                ...
                return {'state':'browser_handoff','instruction':'Open or reuse the persistent X tab in the Codex in-app browser. Sign in only if X itself reports that the saved session expired.'}
            if op=='verify':
                ...
                return s.status()
            if op=='publish':
                raise Stop('codex_in_app_browser_action_required')
