...
class Store:
    ...
    def status(self):
        ...
        return dict(provider='zhihu', route='controlled_browser', routeDriver=VERSION,
                    ...
                    profile=c.get('profile') or self.get('selected_profile'), identitySignals=c.get('identitySignals',[]) if fresh else [],
...
class ZhihuUI:
    ...
    def one(self, locator):
        try: locator.wait_for(state='visible',timeout=10000)
    ...
    def identity(self, account):
        ...
        try:
            ...
            self.one(p.locator('.ProfileHeader').get_by_role('button',name='编辑个人资料',exact=True))
    ...
    def compose(self,text):
        ...
        self.one(self.page.get_by_text('分享此刻的想法...',exact=True)).click()
...
def execute(store, value, approved_hash, ui):
    ...
    with store.lock():
        ...
        safe_resume = bool(existing and existing.get('state') == 'unknown'
                           and existing.get('stage') == 'compose_started')
        if existing and not safe_resume: return existing
        ui.identity(value['profile'])
        before = existing['previousLinks'] if safe_resume else sorted(ui.links(value['profile']))
        # Claim before composition: even autosaved drafts are approval-bound.
        record = (dict(existing, state='attempting', resumedAt=time.time()) if safe_resume else
                  dict(state='attempting',stage='compose_started',manifest=value,hash=key,previousLinks=before,claimedAt=time.time()))
        store.put(key,record)
        try:
            ui.compose(value['text']); ui.identity(value['profile']); ui.assert_text(value['text'])
            record.update(state='submitting', stage='submit_started')
            store.put(key,record)
            ui.submit()
            ...
            record.update(state='unknown',reason='reconcile_before_retry',
                          failureCode=str(error) if isinstance(error,Stop) else type(error).__name__)
